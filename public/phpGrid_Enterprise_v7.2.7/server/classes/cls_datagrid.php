...
class C_DataGrid{
    ...
    private $jq_pgtext;         // e.g. Page {current page} of {total pages}
...
//    private $_file_path;
    ...
    public function __construct($sql, $sql_key=array(), $sql_table='', $db_connection= array()){
        ...
        $this->_ver_num             = 'phpGrid(v7.2.7) {free-jqGrid:v4.13.6, jQuery:v3.3.1, jQuery UI:1.11.2}';
        $this->sys_msg              = null;
        $this->form_width            = 450;
            ...
            if ( ( $col_type === 'D' || $col_type === 'T' ) && !in_array( $col_name, $this->col_readonly ) ){   // do not display datepicker/datetimepicker if readonly
                if((isset($this->col_formats[$col_name]['date']))){
                    ...
                    $editoptions['dataInit'] = '###function(el){$(el).datepicker({
                                                changeMonth: true, changeYear: true,dateFormat:\''. $this->col_formats[$col_name]['date']['datePickerFormat']
                ...
                }elseif((isset($this->col_formats[$col_name]['time']))){

                    $editoptions['dataInit'] = '###function(el){$(el).timepicker({timeInput:true, timeFormat:\'hh:mm TT\'});}###';
    ...
    private function display_style(){
        ...
        if ($this->jq_cellEdit){
            echo '<style type="text/css">' ."\n";
            echo '.table>tbody>tr.success>td, .table>tbody>tr.success>th, .table>tbody>tr>td.success, .table>tbody>tr>th.success, .table>tfoot>tr.success>td, .table>tfoot>tr.success>th, .table>tfoot>tr>td.success, .table>tfoot>tr>th.success, .table>thead>tr.success>td, .table>thead>tr.success>th, .table>thead>tr>td.success, .table>thead>tr>th.success,
                .ui-state-highlight{
                    background-color: initial;
                    outline: 3px solid #377BC8;
                }
        ...
        if ($this->sql_fkey==null) {
            ...
            if (CDN) {
                $this->script_includeonce .= '<script type="text/javascript">
                    if (typeof jQuery == "undefined"){document.write("<script src=\'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js\' type=\'text/javascript\'><\/script>");}
                  </script>' ."\n";
            } else {
                $this->script_includeonce .= '<script type="text/javascript">
                    if (typeof jQuery == "undefined"){document.write("<script src=\''. ABS_PATH .'/js/jquery-3.3.1.min.js\' type=\'text/javascript\'><\/script>");}
                  </script>' ."\n";
            }
            ...
            $this->script_includeonce .= '<script src="'. ABS_PATH .'/js/jquery-migrate-3.0.1.min.js" type="text/javascript"></script>' ."\n";
            ...
            $this->script_includeonce .='<script src="'. ABS_PATH .'/js/datetimepicker/jquery-ui-timepicker-addon.js" type="text/javascript"></script>' ."\n";
            $this->script_includeonce .='<link rel="stylesheet" type="text/css" media="screen" href="'. ABS_PATH .'/js/datetimepicker/jquery-ui-timepicker-addon.css" />' ."\n";
    ...
    public function display_properties_main(){
        ...
        echo    'guiStyle:"'. (strtolower($this->theme_name) === 'bootstrap' ? 'bootstrap' : 'jQueryUI') ."\",\n";
        ...
        */
        ...
        switch($this->edit_mode){
            case 'CELL':
                ...
                echo 'cellurl:"'. ABS_PATH .'/edit.php?dt=json&gn='. $this->jq_gridName .'",'."\n";
    ...
    public function enable_edit($edit_mode = 'FORM', $options='CRUD', $edit_file='edit.php'){
        switch($edit_mode)    {
            case 'CELL':                    // NO LONGER SUPPORTED
                ...
                $this->set_jq_editurl(ABS_PATH .'/edit.php?dt='. $this->jq_datatype .'&gn='.$this->jq_gridName);
    ...
    public function set_col_edittype($col_name, $ctrl_type, $keyvalue_pair=null, $multiple=false, $dataUrl=null){
        if($ctrl_type == "select" || $ctrl_type == "autocomplete") {
            ...
            if (preg_match($regex , $keyvalue_pair, $matches))
            {
                ...
                $keyvalue_pair = "null:;".$select_list; // ADODB_FORCE_VALUE - 3 force nul to Sql NULL
            }
        }
...
//        $this->db->db->debug = $debug;
    ...
    // The grid height is set in the 2nd param of set_scroll(). See method for more info
    public function enable_pagecount($has_pagecount=true){
        if(!$has_pagecount){
            $this->jq_viewrecords = false;
            $this->jq_pgtext = "Page {0}";
        }
